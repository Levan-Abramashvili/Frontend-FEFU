...
function GroupChart({ data }: GroupChartProps) {
  const [series, setSeries] = React.useState<tSeries>({
    'Максимальная посещаемость': true,
    'Средняя посещаемость': false,
    'Минимальная посещаемость': false,
  });
  ...
  const seriesY = Object.entries(series)
    ...
    .map((item) => ({ dataKey: item[0], label: 'Посещаемость (человек)', showMark: false, }));
